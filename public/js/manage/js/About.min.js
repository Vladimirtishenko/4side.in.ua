		...
		render: function () {
			...
			for (var i = 1; i <= 6; i++) {
				if(relate = this.inArray(i)){
					ArrayOfTemplateComponents.push(React.createElement(TemplateForAboutBusy, {infoAdd: this.newAddInformation, edit: this.editState, content: relate, keys: i}));
				} else {
					ArrayOfTemplateComponents.push(React.createElement(TemplateForAbout, {imgAdd: this.newAddImages, infoAdd: this.newAddInformation, keys: i}));
				}
				if(i%2 == 0){
					ArrayOfComponents.push(React.createElement(TemplateOuterForAbout, {element: ArrayOfTemplateComponents}));
