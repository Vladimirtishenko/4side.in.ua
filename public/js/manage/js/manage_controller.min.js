...
function View (MenuItem) {
		...
		render: function () {
			return (
				React.createElement("div", {className: "outer-all-container"}, 
					React.createElement(SideBar, {menu: this.state.menu, lang: this.state.lang, context: this.contextReplace}), 
					React.createElement(Main, {block: this.state.block, lang: this.state.lang, translator: this.state.translator, content: this.state.content, title: this.state.title}), 
					React.createElement(Error, {show: this.state.errorBlock, error: this.state.errorMessage})
				)
			);
		},
		...
		render: function () {
			var MenuArray = this.props.menu ? this.props.menu : null,
				Menu = MenuArray ? MenuArray.map(function (item, i) {
					return item.active ? React.createElement("li", {key: i, className: "active-list"}, React.createElement("span", null, item["name_"+this.props.lang])) : React.createElement("li", {key: i}, React.createElement("a", {onClick: this.props.context, name: item.context, "data-rus-title": item["name_"+this.props.lang], href: "#"}, item["name_"+this.props.lang]))
				}, this) : null;
			...
			return (
				React.createElement("div", {className: "outer-main-block"}, 
					React.createElement("div", {className: "general-block"}, 
						this.props.block ? React.createElement(this.props.block, {content: this.props.content, translator: this.props.translator, lang: this.props.lang, title: this.props.title}) : React.createElement("img", {src: "/images/preloader_1.gif", className: "preload-for-main-block"})
					)
				)
			)
		}
	...
	var Error = React.createClass({displayName: "Error",
		render: function(){
			...
			return (React.createElement("div", {className: shadow}, React.createElement("p", {className: "error-block-message"}, "Этот сервис сейчас недоступен обновите страницу или попробуйте позже ", React.createElement("br", null), " Приложение вернуло ошибку с текстом: ", React.createElement("b", null, message))));
		}
	})
	...
	*/
	...
	ReactDOM.render(React.createElement(Outer, null), document.getElementById('container-for-content'));
